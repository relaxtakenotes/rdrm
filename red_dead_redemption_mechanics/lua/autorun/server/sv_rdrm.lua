...
net.Receive("rdrm_request_change_state", function(len, ply)
	...
	if game.SinglePlayer() then
		...
		if not rdrm.in_killcam and smooth_ending and not rdrm.in_deadeye[ply] then
			timescale_lerp = 0
		end
...
if game.SinglePlayer() then
	hook.Add("EntityTakeDamage", "rdrm_detect_damage", function(ent, dmg)
		if game.SinglePlayer() and ent:IsNPC() and dmg:GetAttacker():IsPlayer() then
